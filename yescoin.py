...
def process_account(token):
    ...
    if game_info and game_info['data'].get('specialBoxLeftRecoveryCount', 0) > 0:
        ...
        print(f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : Không có rương nào có sẵn          ", flush=True)
    time.sleep(2)
    print(f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Đang cố gắng phục hồi...", end="", flush=True)
    game_info = fetch_account_build_info(token)
    if game_info and game_info['data'].get('coinPoolLeftRecoveryCount', 0) > 0:
        ...
        print(f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Không có khả năng phục hồi", flush=True)
    time.sleep(2)
    print(f"{random.choice(available_colors)+Style.BRIGHT}\r[ Free Chest ] : Đang cố gắng thu thập..", end="", flush=True)
...
def main():
    ...
    while True:
        ...
        for detik in range(waktu_tunggu, 0, -1):
            sys.stdout.write(f"\r{Fore.CYAN} Đang chờ lần yêu cầu tiếp theo vào {Fore.CYAN}{Fore.WHITE}{detik // 60} phút {Fore.WHITE}{detik % 60} giây ")
            sys.stdout.flush()
            time.sleep(1)
        sys.stdout.write("\rThời điểm cho yêu cầu tiếp theo đã đến!                                                          \n")
